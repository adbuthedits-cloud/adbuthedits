...
import { faSave, faEdit, faArrowLeft, faPlus, faTimes, faTrash, faVideo, faPlay, faPencilAlt, faCloudUploadAlt, faSpinner, faFileUpload, faCheckCircle } from '@fortawesome/free-solid-svg-icons';
    ...
    return (
        <>
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    <h1 className="text-3xl font-bold text-white tracking-tight">View Product</h1>
                    <p className="text-gray-400">Viewing read-only product details</p>
                </div>
            </div>

            <div className="flex justify-between items-center bg-[#1E1628] p-6 rounded-2xl border border-[#2d1b4e] shadow-[0_4px_20px_rgba(0,0,0,0.2)] mb-8">
                <div>
                    <span className="text-gray-500 text-xs font-bold uppercase tracking-widest block mb-1">Created On</span>
                    <span className="text-[#a78bfa] font-mono font-medium">{formData.createdAt ? new Date(formData.createdAt).toLocaleString('en-US', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' }) : '-'}</span>
                </div>
                <div className="text-right">
                    <span className="text-gray-500 text-xs font-bold uppercase tracking-widest block mb-1">Last Updated</span>
                    <span className="text-emerald-400 font-mono font-medium">{formData.updatedAt ? new Date(formData.updatedAt).toLocaleString('en-US', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' }) : '-'}</span>
                </div>
            </div>
                ...
                <div className="bg-[#1E1628] rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.2)] border border-[#2d1b4e] p-8 space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex gap-2">
                                <input name="thumbnail" value={formData.thumbnail} onChange={handleChange} className="flex-1 p-3 bg-[#2d1b4e] border border-transparent rounded-xl text-gray-200 focus:ring-2 focus:ring-[#a78bfa]/30 focus:border-[#a78bfa]/50 outline-none transition-all placeholder-gray-500" />
