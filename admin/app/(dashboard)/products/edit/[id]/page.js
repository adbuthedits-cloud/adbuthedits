...
import { faSave, faEdit, faArrowLeft, faPlus, faTimes, faTrash, faVideo, faPlay, faPencilAlt, faCloudUploadAlt, faSpinner, faFileUpload, faCheckCircle, faFolderOpen, faExpand, faSync, faCompress } from '@fortawesome/free-solid-svg-icons';
...
function EditProduct() {
    ...
    // Per-video compression status: { [id]: { status: 'compressing'|'done'|'skipped', pct: number } }
    const [videoCompressStatus, setVideoCompressStatus] = useState({});

    // Tracks R2 URLs of media files to delete physically when form submission succeeds
    ...
    const handleFileUpload = async (file, subfolder = 'image', explicitKey = null, onProgress = null) => {
        if (!file) return { url: null, timedOut: false, error: null };
        try {
            const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000';
            ...
            const token = getAuthToken();
            if (!token) return { url: null, timedOut: false, error: 'No token' };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            for (let i = 0; i < videos.length; i++) {
                ...
                // Upload compressed _web version
                if (hasCompressed) {
                    let webKey = null;
                    if (origResult.url) {
                        try {
            ...
            // Clean up physically from R2 storage now that the database update has succeeded
            if (pendingDeletions.length > 0) {
                try {
                    await Promise.allSettled(
                        pendingDeletions.map(async (fileUrl) => {
                            await axios.delete(`${apiUrl}/api/admin/delete-media`, {
                                headers: { Authorization: `Bearer ${token}` },
                                data: { fileUrl }
                            });
                        })
                    ...
                    console.error("Failed to clean up deleted files from storage:", delErr);
                }
            }

            setIsDirty(false);
            toast.success(isDraftSubmit ? "Draft updated successfully!" : "Product updated successfully!");
            setSaving(false);
                            ...
                            {!uploadingThumbnail && (formData.thumbnail || thumbnailPreview) && (
                                <div className="mt-3 flex items-start gap-3">
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => {
                                            const isLocal = formData.thumbnail instanceof File || (thumbnailPreview && thumbnailPreview.startsWith('blob:'));
                                            const isR2Asset = !isLocal && typeof formData.thumbnail === 'string' && formData.thumbnail.startsWith('http');
                                            
                                            const message = isR2Asset
                                                ? "Are you sure you want to remove the current thumbnail? (It will be permanently deleted from storage when you save your changes.)"
                                                : "Are you sure you want to remove the current thumbnail?";
                                            
                                            if (!window.confirm(message)) return;

                                            if (isR2Asset) {
                                                setPendingDeletions(prev => [...prev, formData.thumbnail]);
                                            }
                                            ...
                                            <button 
                                                ...
                                                onClick={() => {
                                                    ...
                                                    const message = isLocal
                                                        ...
                                                        : "Are you sure you want to remove this image? (It will be permanently deleted from storage when you save your changes.)";
                                                    if (!window.confirm(message)) return;
                                        ...
                                        <button
                                            ...
                                            onClick={(e) => {
                                                ...
                                                if (isVidCompressing) {
                                                    ...
                                                    const isLocal = vid instanceof File;
                                                    const isR2Asset = !isLocal && typeof vid === 'string' && !vid.includes('youtube.com') && !vid.includes('youtu.be') && !vid.includes('vimeo.com');
                                                    
                                                    const message = isR2Asset
                                                        ? "Are you sure you want to remove this video? (It will be permanently deleted from storage when you save your changes.)"
                                                        : "Are you sure you want to remove this video?";
                                                    
                                                    if (!window.confirm(message)) return;

                                                    if (isR2Asset) {
                                                        setPendingDeletions(prev => [...prev, vid]);
                                                    }
                                                }
                            ...
                            {formData.resource_file && (
                                <div className="p-4 bg-[#1E1628] rounded-xl border border-green-500/20 flex items-center justify-between">
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => {
                                            const isLocal = formData.resource_file instanceof File;
                                            const isR2Asset = !isLocal && typeof formData.resource_file === 'string' && formData.resource_file.startsWith('http');
                                            
                                            const message = isR2Asset
                                                ? "Are you sure you want to remove this resource package? (It will be permanently deleted from storage when you save your changes.)"
                                                : "Are you sure you want to remove this resource package?";
                                            
                                            if (!window.confirm(message)) return;

                                            if (isR2Asset) {
                                                setPendingDeletions(prev => [...prev, formData.resource_file]);
                                            }
