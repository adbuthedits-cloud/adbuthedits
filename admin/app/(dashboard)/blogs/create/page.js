...
function CreateBlogContent() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000';
            await axios.post(`${apiUrl}/api/admin/blogs`, dataToSubmit, {
                headers: { Authorization: `Bearer ${token}` }
            });
            ...
            console.error(error);
            const errMsg = error.response?.data?.error || error.message || 'Failed to post blog';
            alert(`Error: ${errMsg}`);
        } finally {
            setLoading(false);
        }
                ...
                <div className="flex flex-col gap-6 overflow-y-auto pr-2 custom-scrollbar pb-20">
                    <div className="bg-[#1E1628] rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.2)] border border-[#2d1b4e] p-6 space-y-6">
                        <div className="space-y-4 border-b border-[#2d1b4e] pb-6">
                            <input name="title" required onChange={handleChange} className="w-full p-3 border border-[#2d1b4e] text-2xl rounded-xl font-bold bg-[#130C1C] outline-none placeholder-gray-600 text-white" placeholder="Post Title" />
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-4">
                                    <input name="slug" required onChange={handleChange} className="w-full p-3 bg-[#130C1C] border border-[#2d1b4e] rounded-xl text-sm font-mono text-[#a78bfa] outline-none placeholder-gray-600" placeholder="slug-url" />
                                    <select
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <input name="author" onChange={handleChange} className="p-3 bg-[#130C1C] border border-[#2d1b4e] rounded-xl text-sm text-gray-300 outline-none placeholder-gray-600" placeholder="Author Name" />
                                <div className="relative">
                                    <input type="date" name="post_date" value={formData.post_date} onChange={handleChange} className="w-full p-3 bg-[#130C1C] border border-[#2d1b4e] rounded-xl text-sm text-white outline-none placeholder-gray-600 appearance-none custom-date-input" />

                                </div>
                            </div>
                            <input name="tags" onChange={handleChange} className="w-full p-3 bg-[#130C1C] border border-[#2d1b4e] rounded-xl text-sm text-gray-300 outline-none placeholder-gray-600" placeholder="Tags (comma separated)" />
                            ...
                            <div className="pt-6 border-t border-[#2d1b4e] space-y-4">
                                ...
                                <div className="space-y-3">
                                    <input name="meta_title" onChange={handleChange} className="w-full p-3 bg-[#130C1C] border border-[#2d1b4e] rounded-xl text-sm text-gray-300 outline-none placeholder-gray-600" placeholder="Meta Title (SEO)" />
                                    <textarea name="meta_description" onChange={handleChange} rows="3" className="w-full p-3 bg-[#130C1C] border border-[#2d1b4e] rounded-xl text-sm text-gray-300 outline-none placeholder-gray-600 resize-none" placeholder="Meta Description"></textarea>
                                    <div className="grid grid-cols-2 gap-4">
                                        <input name="meta_keywords" onChange={handleChange} className="p-3 bg-[#130C1C] border border-[#2d1b4e] rounded-xl text-sm text-gray-300 outline-none placeholder-gray-600" placeholder="Keywords" />
                                        <input name="canonical_url" onChange={handleChange} className="p-3 bg-[#130C1C] border border-[#2d1b4e] rounded-xl text-sm text-gray-300 outline-none placeholder-gray-600" placeholder="Canonical URL" />
                                    </div>
                                </div>
                            </div>
...
export default withPermission(CreateBlog, 'blogs');
